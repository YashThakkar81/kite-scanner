...
if change_percent > 2.0:
    ...
    # Import the connection tool
from streamlit_gsheets import GSheetsConnection

# Initialize the connection
conn = st.connection("gsheets", type=GSheetsConnection)

def log_to_sheets(df):
    # Filter only the "BREAKOUT" rows
    breakouts = df[df['Status'] == '🚀 BREAKOUT']
    
    if not breakouts.empty:
        # Get existing data from the sheet
        existing_data = conn.read(spreadsheet=st.secrets["GSHEET_URL"])
        
        # Combine existing and new breakout data
        updated_data = pd.concat([existing_data, breakouts], ignore_index=True)
        
        # Write it back to the sheet
        conn.update(spreadsheet=st.secrets["GSHEET_URL"], data=updated_data)
        st.toast("✅ Breakout logged to Google Sheets!")
